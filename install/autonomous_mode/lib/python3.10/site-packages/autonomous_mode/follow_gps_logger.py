...
from autonomous_mode.utilities import latLonYaw2Geopose
...
class YamlWaypointParser:
    ...
    def get_wps(self):
        ...
        for wp in self.wps_dict["waypoints"]:
            latitude, longitude,alt = wp["latitude"], wp["longitude"], wp["alt"]
            gepose_wps.append(latLonYaw2Geopose(latitude, longitude, alt))
        return gepose_wps
...
def main():
    ...
    default_yaml_file_path = os.path.join(get_package_share_directory(
        "autonomous_iit"), "config", "gps_waypoints.yaml")
    if len(sys.argv) > 1:
