...
class navigating_point(Node):
    def __init__(self, shared_variables):
        ...
        self.target_wp=[ (47.3978405,8.5456752),(47.3978856,8.5455423),(47.397706799999995,8.545874699999999),(47.397767599999995,8.545648)  ]
        self.target_angle=self.drone_curr_angle
        self.present_wp=0
        self.x_ned=0.0
        self.y_ned=0.0
    ...
    def takeoff(self,take_off_h:float,yaw_angle:float):
        ...
        if diff_z>0.3:
            self.publish_pos_vel_setpoint(current_x,current_y,take_off_h,0.0,0.0,0.0,yaw_angle)
            print(f"{yaw_angle}")
            return False
    ...
    def publish_pos_vel_setpoint(self, x: float, y: float, z: float,x_vel:float,y_vel:float,z_vel:float):
    ...
    def navigate_to_point(self,target_pos_x:float,target_pos_y:float,target_angle:float):
        ...
        diff_angle=abs(abs(current_angle)-abs(target_angle))
    ...
    def timer_callback_2(self) -> None:
        ...
        if  self.shared_variables.vehicle_local_pos.z>self.take_off_h and self.shared_variables.vehicle_status_.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            print("l")
            self.publish_pos_vel_setpoint(current_x,current_y,self.take_off_h-0.5,0.0,0.0,0.0)
        

        elif self.shared_variables.vehicle_local_pos.z<=self.take_off_h and self.shared_variables.vehicle_status_.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD:
            # diff_x=abs(abs(current_x)-abs(5.0))
            # if diff_x>0.3:
            #     self.publish_pos_vel_setpoint(5.0,current_y,self.take_off_h-0.5,0.0,0.0,0.0,self.drone_curr_angle)
            # else:
            self.land()
            self.landed=True
